...
parser = argparse.ArgumentParser(description='birealnet18')
...
origin_model = birealnet18()
checkpoint = torch.load('pretrained/imagenet/naive/model_best.pth.tar')
...
for m in origin_model.modules():
    if isinstance(m, birealnet.HardBinaryConv):
        i = i + 1
        if i % 2 == 0:
...
for s in range(channels):
    ...
    clustering = DBSCAN(eps=0.008, min_samples=5, metric='cosine').fit(X)
