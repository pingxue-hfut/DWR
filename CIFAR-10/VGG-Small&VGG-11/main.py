...
model_names = sorted(name for name in vgg11.__dict__
    if name.islower() and not name.startswith("__")
                     and name.startswith("vgg")
                     and callable(vgg11.__dict__[name]))

parser = argparse.ArgumentParser(description='PyTorch ImageNet Training')
parser.add_argument('--arch', '-a', metavar='ARCH', default='vgg_11',
                    choices=model_names,
...
def main():
    ...
    model = torch.nn.DataParallel(vgg11.__dict__[args.arch]())
    model.cuda()
